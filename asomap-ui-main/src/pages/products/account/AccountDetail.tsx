...
import type { IAccountData } from '@/interfaces';
...
const AccountDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAccountData = async () => {
      try {
        ...
        // Buscar la cuenta por slug (convertir título a slug)
        const account = accounts.find(acc => {
          const accountSlug = acc.title.toLowerCase()
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/(^-|-$)/g, '');
  ...
  const { bannerImage, accountImage, title, description, features, requirements, benefits } = accountData;
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="lg:flex lg:items-start lg:justify-between">
              <div className="lg:w-1/2 pr-0 lg:pr-8 mb-8 lg:mb-0">
                ...
                {/* Sección de Tarifas */}
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-primary mb-4 sm:mb-6">Tarifas</h2>
                  <ul className="space-y-2 sm:space-y-4">
